...
## 별이 빛나는 밤에 — 원본 샘플링 방식 전환 (2026-08-08)

- 절차적 근사를 버리고 원본 회화 이미지를 직접 재현하는 방식으로 전환.
- `scripts/extract-painting.py`(Pillow+numpy)가 원본에서 144x104 색상 맵과
  구조 텐서 기반 붓결 방향장(배각 2θ 벡터, int8)을 추출해
  `lib/scenes/starryNightData.ts`(base64 내장, ~100KB)로 생성.
- 씬은 색상 맵을 쌍선형 샘플링(+채도·밝기 보정, 붓결 방향 미세 표류),
  flow는 방향장을 쌍선형 보간 — 붓터치가 원화의 실제 스트로크 결을 따라 흐른다.
- 우측 스크림을 0.9→0.55로 완화 (달이 가려지던 문제).
- 다른 씬에도 같은 파이프라인 적용 가능 (scripts/extract-painting.py의 SRC/OUT만 교체).

## 전 작품 원본 샘플링 전환 + UI 미니멀화 (2026-08-08)

- 나머지 5개 작품(해바라기·카페 테라스·아를의 침실·자화상·밀밭의 사이프러스)도
  Wikimedia Commons 퍼블릭 도메인 원본을 받아 같은 파이프라인으로 전환.
- 공용 모듈 `lib/scenes/painting.ts` 신설 — 씬 파일은 3줄 래퍼가 됨.
- extract-painting.py를 CLI화 (--src/--out/--crop), 종횡비 유지 최대 변 144px.
- UI 텍스트 전면 제거 (사용자 요청): 브랜드 바·라이브·에센스·킥커·메타·좋아요/공유·노트 삭제.
  작품 제목(h1)과 도트·화살표 네비게이션만 유지. Work 타입도 title/cell/scene/flow로 슬림화.

## 작품 정보 패널 복원 (2026-08-09)

- 사용자 요청으로 패널에 정보 재추가: 원제·연도·소장처(kicker), 제목, 한 줄 설명,
  작가 정보(빈센트 반 고흐 · 1853–1890 · 후기 인상주의), 생성 모델 표기
